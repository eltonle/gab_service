...
<div class="row ">
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                @php
                $payment = App\Models\ServicePayment::where('service_id',$invoice->id)->first();
                @endphp
                ...
                <hr>
                <form method="post" action="{{ route('service.approve.store',$invoice->id) }}">
                    @csrf
                    <table border="1" width="100%" class="table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr class="text-center">
                                <input type="hidden" name="technical_id[]" value="{{ $details->technical_id }}">
                                <input type="hidden" name="task_id[]" value="{{ $details->task_id }}">
                                <input type="hidden" name="selling_qty[{{ $details->id }}]" value="{{ $details->selling_qty }}">
                                <td>{{ $key+1 }}</td>
                                <td>{{ $details['technical']['name'] }}</td>
                                <td>{{ $details['task']['name'] }}</td>
                                <!-- <td class="text-center" style="background: #ddd;padding: 1px;">{{ $details['task']['quantity'] }}</td> -->
